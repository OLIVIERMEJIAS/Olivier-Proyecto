...
namespace Olivier_Proyecto
{
    public partial class frmFunciones : Form
    {
        ...
        LNCalificacion lnc = new LNCalificacion(Config.getCadConexion);
        LNDetalleHorario lnd = new LNDetalleHorario(Config.getCadConexion);
        EEstudiante est;
        int materiaId;
        int estudianteId;
        int fila = 0;
        ...
        {
            listar = false;
            est = lne.existeCedUsuario(txtCedCalif.Text);
            estudianteId = est.Id;
            if (est.Nombre == "")
            {
            ...
            {
                txtNomCalif.Text = $"{est.Nombre} {est.Apellido1} {est.Apellido2}";
                DataTable datos;
                txtMateriaCalif.Text = lnm.accederANombre(EGlobal.profesorId).Nombre;
                materiaId = lnm.accederANombre(EGlobal.profesorId).MateriaId;
                datos = lnc.listarPorEstudiante(est.Id, materiaId);
                dgvCalificaciones.DataSource = datos;
                dgvCalificaciones.Columns[0].HeaderText = "EstudianteID";
                dgvCalificaciones.Columns[1].HeaderText = "MateriaID";
                dgvCalificaciones.Columns[2].HeaderText = "Calificación";
                dgvCalificaciones.Columns[3].HeaderText = "Fecha Ingreso";
                dgvCalificaciones.Columns[4].HeaderText = "Estado";
                dgvCalificaciones.Columns[5].HeaderText = "Trimestre";

                fila = 0;
            }
        }
        private void btnElimCalif_Click(object sender, EventArgs e)
        {
            if (!guardar)
            {
                bool result;

                string fech = DateTime.Parse(dgvCalificaciones[3, fila].Value.ToString()).ToString("yyyy-MM-dd");
                ECalificacion calif = new ECalificacion(int.Parse(dgvCalificaciones[0, fila].Value.ToString()),
                    int.Parse(dgvCalificaciones[1, fila].Value.ToString()),
                    decimal.Parse(dgvCalificaciones[2, fila].Value.ToString()), fech,
                        dgvCalificaciones[4, fila].Value.ToString(),
                        int.Parse(dgvCalificaciones[5, fila].Value.ToString()));
                try
                {
                    result = lnc.eliminarCalificacion(calif);
                    if (result)
                    {
                        MessageBox.Show("Calificación eliminada con éxito");
                        if (!listar)
                        {
                            btnBuscarCedCalif_Click(sender, e);
                        }
                        else
                        {
                            btnListarCalif_Click(sender, e);
                        }
                        guardar = true;
                    }
                    else
        ...
        private void btnListarCalif_Click(object sender, EventArgs e)
        {
            ...
            {
                DataTable datos;
                datos = lnc.listarPorSeccion(cmbSecCalif.Text);
                dgvCalificaciones.DataSource = datos;
                dgvCalificaciones.Columns[0].HeaderText = "EstudianteID";
                dgvCalificaciones.Columns[1].HeaderText = "MateriaID";
                dgvCalificaciones.Columns[2].HeaderText = "Calificación";
        ...
        private void btnGuardarCalif_Click(object sender, EventArgs e)
        {
            ...
            {
                bool result;
                ECalificacion calif = new ECalificacion(estudianteId, materiaId, decimal.Parse(txtCalif.Text),cmbEstadoCalif.Text,int.Parse(cmbTrim.Text));
                try
                {
                    result = lnc.agregarCalificacion(calif);
                    if (result)
                    {
                        guardar = true;
                        ...
                        MessageBox.Show("No fue posible asignar la calificación");
                }
                catch (Exception ex)
                {
                    guardar = true;
            ...
            else
            {
                ...
                ECalificacion calif = new ECalificacion(int.Parse(dgvCalificaciones[0, fila].Value.ToString()),
                    int.Parse(dgvCalificaciones[1, fila].Value.ToString()),
                    decimal.Parse(txtCalif.Text), fech,
                        cmbEstadoCalif.Text,
                        int.Parse(dgvCalificaciones[5, fila].Value.ToString()));
                try
                {
                    result = lnc.actualizarCalificacion(calif);
                    if (result)
                    {
                        ...
                        MessageBox.Show("No fue posible actualizar la calificación");
                }
                catch (Exception ex)
                {
        ...
        {
            fila = dgvCalificaciones.CurrentRow.Index;
            guardar = false;
            MessageBox.Show("Se ha abierto el espacio para modificar el estado de la calificación y dar guardar," +
                "o solo dar en eliminar para deshacer la calificación");
        ...
        private void btnGenerar_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                result = lnd.agregarDetallesHorarrio(generarHorarios());
                if (result)
                {
                    MessageBox.Show("Horarios generados correctamente");
                }
            }
            catch (Exception ex)
            {
        ...
        private void btnListarHor_Click(object sender, EventArgs e)
        {
            DataTable datos;
            datos = lnd.listarPorSeccion(cmbSecHor.Text);
            dgvHor.DataSource = datos;
            dgvHor.Columns[0].HeaderText = "DÍA";
            dgvHor.Columns[1].HeaderText = "MATERIA";
            dgvHor.Columns[2].HeaderText = "HORA INICIO";
            dgvHor.Columns[3].HeaderText = "HORA FIN";
        }
        #endregion
    }
}
